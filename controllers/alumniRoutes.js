...
alumniApp.post("/alumni/new-job", checkAuth, (req, res) => {
  ...
  newJob
    .save()
    .then(() => res.redirect("/alumni/view-job"))
    .catch((err) => console.log(err));
});

alumniApp.get("/alumni/view-job", checkAuth, (req, res) => {
  var jobs;
  let email = req.user.email;
  job.find({}, (err, data) => {
    if (err) {
    ...
    res.render("jobs", { data: jobs, user_email: email});
  });
});

alumniApp.post("/alumni/delete-job", (req, res) => {
  const id = req.body.id;
  job.findOneAndRemove({ _id: id }, (err, doc) => {
    res.redirect("/alumni/view-job");
  });
});
